...
class ConversationContextManager:
    ...
    async def add_to_conversation_history(self, user_id: str, message: Dict[str, Any]):
        """Add message to conversation history with support for new structured format."""
        try:
            message['timestamp'] = datetime.now().isoformat()
            
            # Ensure the message has all required fields for the new format
            if message.get('type') == 'assistant':
                # Make sure structured response fields are present
                if 'main_answer' not in message:
                    message['main_answer'] = ''
    ...
    async def get_structured_conversation_history(self, user_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Get conversation history with structured response data."""
        try:
            history = await self.get_conversation_history(user_id, limit)
            
            # Ensure each assistant message has the structured fields
            for message in history:
                if message.get('type') == 'assistant':
                    if 'main_answer' not in message:
                        message['main_answer'] = message.get('content', '')
                    if 'context' not in message:
                        message['context'] = ''
            ...
            print(f"Error getting structured conversation history: {str(e)}")
            return []
    ...
    def _get_default_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'response_preference': 'structured'  # NEW: Track user preference for structured responses
        }
    ...
    async def extract_context_from_query(self, query: str, current_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ✅ Detect response preference hints
        if re.search(r"\bsimple\b|\bshort\b|\bbrief\b", query_lower):
            extracted["response_preference"] = "concise"
        elif re.search(r"\bdetailed\b|\bexplain\b|\belaborate\b", query_lower):
            extracted["response_preference"] = "detailed"
    ...
    async def clear_conversation_history(self, user_id: str):
        ...
        try:
            if self.redis_client:
                await self.redis_client.delete(f"conversation:{user_id}")
            else:
                history_key = f"conversation:{user_id}"
                if history_key in self.context_cache:
                    del self.context_cache[history_key]
    ...
    async def get_conversation_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            history = await self.get_conversation_history(user_id, 100)  # Get more for stats
