...
const AdminDashboard = () => {
  ...
  const renderOverview = () => (
    <div>
      <h2 style={{ marginBottom: 'var(--spacing-xl)' }}>{getTranslation('dashboardOverview', language)}</h2>
      {/* Stats Cards */}
        ...
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, color: 'var(--color-text-secondary)' }}>{getTranslation('totalFarmers', language)}</p>
            </div>
          </div>
        </div>
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, color: 'var(--color-text-secondary)' }}>{getTranslation('totalRetailers', language)}</p>
            </div>
          </div>
        </div>
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, color: 'var(--color-text-secondary)' }}>{getTranslation('totalLabourers', language)}</p>
            </div>
          </div>
        </div>
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, color: 'var(--color-text-secondary)' }}>{getTranslation('totalTransactions', language)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 'var(--spacing-lg)' }}>
        <div style={cardStyle}>
          <h3 style={{ marginBottom: 'var(--spacing-lg)' }}>{getTranslation('recentTransactions', language)}</h3>
            ...
            {recentTransactions.map(transaction => (
              <div key={transaction.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <h4 style={{ margin: 0, fontSize: '0.9rem' }}>{getTranslation(transaction.type.replace(/\s/g, ''), language) || transaction.type}</h4>
        ...
        <div style={cardStyle}>
          <h3 style={{ marginBottom: 'var(--spacing-lg)' }}>{getTranslation('systemStatus', language)}</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{getTranslation('serverStatus', language)}</span>
              <span style={{ color: 'var(--color-success)', fontWeight: '600' }}>🟢 {getTranslation('online', language)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{getTranslation('database', language)}</span>
              <span style={{ color: 'var(--color-success)', fontWeight: '600' }}>🟢 {getTranslation('healthy', language)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{getTranslation('apiServices', language)}</span>
              <span style={{ color: 'var(--color-success)', fontWeight: '600' }}>🟢 {getTranslation('running', language)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{getTranslation('emailService', language)}</span>
              <span style={{ color: 'var(--color-success)', fontWeight: '600' }}>🟢 {getTranslation('active', language)}</span>
            </div>
          </div>
        </div>
      </div>
